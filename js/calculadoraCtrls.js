...
calcController = angular.module('calcController', []);

calcController.directive("bnDocumentKeypress", function($document, $parse) {
  var linkFunction;
  return linkFunction = function($scope, $element, $attributes) {
...
calcController.controller("calculadoraController", function($scope) {
  $scope.calcInput = '0';
  $scope.memory = 0;
  $scope.stuff = [0];
  ...
  return $scope.memoryIndicator = function(show) {
    if (!show) {
      return $('#memoryBtn').removeClass('memoryOn');
    } else if (!$('#memoryBtn').hasClass('memoryOn')) {
      return $('#memoryBtn').addClass('memoryOn');
    }
  };
});
